...
func _init(size: Vector2i):
	...
	tile_size = size
	tile_mask = Image.create(tile_size.x, tile_size.y, 
...
func get_bounding_rect() -> Rect2i:
	match mode:
		MODE.BOTH:
			...
			bounding_rect = bounding_rect.expand(cross_diagonal)
			bounding_rect = bounding_rect.grow_individual(
				0, 0, tile_size.x, tile_size.y)
...
func get_nearest_tile(point: Vector2i, 
					  tile_positions :Array[Vector2i]) -> Rect2i:
	tile_positions.append(Vector2i.ZERO)
	...
	if final.is_empty():
		return Rect2i(Vector2i.ZERO, tile_size)
	final.sort_custom(
		func(a: Rect2i, b: Rect2i): return a.position.y < b.position.y)
	return final[0]


func get_canon_position(position: Vector2i, 
						tile_positions: Array[Vector2i]) -> Vector2i:
	if mode == MODE.NONE:
		return position
	var nearest_tile := get_nearest_tile(position, tile_positions)
	if nearest_tile.has_point(position):
		position -= nearest_tile.position
	return position


func get_point_in_tiles(pixel: Vector2i,
						tile_positions :Array[Vector2i]) -> Array[Vector2i]:
	tile_positions.append(Vector2i.ZERO)
...
func has_point(point: Vector2i, tile_positions: Array[Vector2i]) -> bool:
	tile_positions.append(Vector2i.ZERO)  # The central tile is included manually
