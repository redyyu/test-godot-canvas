...
signal selection_map_changed(sel_map)
...
const KEY_MOVE_ACTION_NAMES: PackedStringArray = [
	"ui_up",
	"ui_down",
	"ui_left",
	"ui_right"
...
var big_bounding_rectangle :Rect2i = Rect2i()
...
var resize_keep_ratio :bool = false

var mirror_view :bool = false
var camera_zoom: Vector2 = Vector2.ZERO
var current_size :Vector2i = Vector2i.ZERO
var current_layer
var selection_offset :Vector2 = Vector2.ZERO
var selection_map :SelectionMap
var has_selection :bool = false
var grid_size: Vector2i = Vector2i.ZERO
var selection_border_color_1 :Color = Color.WHITE
var selection_border_color_2 :Color = Color.WHITE
...
func _input(event: InputEvent) -> void:
	...
	if mirror_view:
		image_current_pixel.x = current_size.x - image_current_pixel.x
	...
	if not current_layer.can_layer_get_drawn():
		return
		...
		if event.pressed:
			if gizmo_hover and not dragged_gizmo: # Select a gizmo
				...
				if not is_moving_content:
					...
					temp_rect = big_bounding_rectangle
					# If prev_temp_rect, which used to be the previous temp_rect, 
					# has negative size,
				...
				temp_rect_pivot = (temp_rect.position + ((
					temp_rect.end - temp_rect.position) / 2))
...
func _draw() -> void:
	...
	var scale_tmp := scale
	if mirror_view:
		position_tmp.x = position_tmp.x + current_size.x
		scale_tmp.x = -1
	draw_set_transform(position_tmp, rotation, scale_tmp)
	for gizmo in gizmos:  # Draw gizmos
		draw_rect(gizmo.rect, selection_border_color_2)
		...
		draw_rect(filled_rect, selection_border_color_1) # Filled white square
...
func _update_gizmos() -> void:
	...
	var size: Vector2 = Vector2.ONE / camera_zoom * 10
	...
	gizmos[2].rect = Rect2(
		Vector2(rect_end.x, rect_pos.y - size.y), size)
	gizmos[3].rect = Rect2(
		Vector2(rect_end.x, (rect_end.y + rect_pos.y - size.y) / 2), size)
	gizmos[4].rect = Rect2(rect_end, size)
	gizmos[5].rect = Rect2(
		Vector2((rect_end.x + rect_pos.x - size.x) / 2, rect_end.y), size)
...
func update_on_zoom() -> void:
	var size := maxi(
		selection_map.get_size().x,
		selection_map.get_size().y
	)
	marching_ants_outline.material.set_shader_parameter("width", 1.0 / camera_zoom.x)
	marching_ants_outline.material.set_shader_parameter("frequency", 
														camera_zoom.x * 10 * size / 64)
	for gizmo in gizmos:
...
func _gizmo_resize() -> void:
	...
	if shape_center:
		...
		if dir.x != 0 and dir.y != 0:  # Border gizmos
			temp_rect.size = ((image_current_pixel - temp_rect_pivot) 
							  * 2.0 * Vector2(dir))
	...
	if shape_perfect or resize_keep_ratio:
		# Maintain aspect ratio
		var end_y = temp_rect.end.y
		if dir == Vector2i(1, -1) or dir.x == 0:
			# Top right corner, center top and center bottom
...
func resize_selection() -> void:
	...
	selection_map_copy.resize_bitmap_values(size, temp_rect.size)
	selection_map_changed.emit(selection_map_copy)
	queue_redraw()
...
func _gizmo_rotate() -> void:  # Does not work properly yet
	...
	if angle == prev_angle:
		return
	prev_angle = angle
	var pivot = Vector2(big_bounding_rectangle.size.x / 2.0, 
						big_bounding_rectangle.size.y / 2.0)
	preview_image.copy_from(original_preview_image)
	if original_big_bounding_rectangle.position != big_bounding_rectangle.position:
		preview_image.fill(Color(0, 0, 0, 0))
		var pos_diff = ((original_big_bounding_rectangle.position -
						 big_bounding_rectangle.position).abs())
		preview_image.blit_rect(original_preview_image,
								Rect2(Vector2.ZERO, preview_image.get_size()),
								pos_diff)
	DrawingAlgos.nn_rotate(preview_image, angle, pivot)
	preview_image_texture = ImageTexture.create_from_image(preview_image)

	var bitmap_image = original_bitmap
	var bitmap_pivot = (original_big_bounding_rectangle.position 
						+ ((original_big_bounding_rectangle.end - 
			 				original_big_bounding_rectangle.position) / 2))
	DrawingAlgos.nn_rotate(bitmap_image, angle, bitmap_pivot)
	
	selection_map_changed.emit(bitmap_image)
...
func transform_content_confirm() -> void:
	...
	for cel in _get_selected_draw_cels():
		...
		cel_image.blit_rect_mask(
			src,
			src,
			Rect2(Vector2.ZERO, selection_map.get_size()),
