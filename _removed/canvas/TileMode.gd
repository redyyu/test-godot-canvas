...
var tiles :Tiles
var draw_center :bool = false
var tilemode_opacity :float = 1.0
var mirror_view :bool = false
var currently_visible_frame: SubViewport
...
func _draw():
	...
	if mirror_view:
		var position_tmp = Vector2(g.current_project.size.x, 0)
		var scale_tmp := Vector2(-1, 1)
		draw_set_transform(position_tmp, 0, scale_tmp)

	var modulate_color := Color(tilemode_opacity, tilemode_opacity, 
								tilemode_opacity, tilemode_opacity) 
	
	var curr_frame_tx: Texture2D = currently_visible_frame.get_texture()
	for pos in positions:
		draw_texture(curr_frame_tx, pos, modulate_color)
...
func get_tile_positions() -> Array[Vector2i]:
	...
	if defaulted_tiles == null:
		defaulted_tiles = g.current_project.tiles
